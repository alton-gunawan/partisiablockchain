//! Provides a sorted vector-based map backed by a simple [`Vec<Entry<K,V>>`] for use in
//! PBC smart contracts. The map provides constant time serialization/deserialization if the
//! entries is serializable by copy (Constant size determinable at compile time).
//!
//! The map provides amortized `O(n)` insert and `O(log(n))` lookup.
...
///
/// ### Fields:
/// * `entries`: [`Vec<Entry<K,V>>`], the entries in the map.
...
impl<K: Ord, V, const N: usize> From<[(K, V); N]> for SortedVecMap<K, V> {
    ...
    fn from(mut arr: [(K, V); N]) -> Self {
        ...
        let arr = arr.map(|(key, value)| Entry { key, value });
        SortedVecMap {
            entries: Vec::from(arr),
        }
    }
}
